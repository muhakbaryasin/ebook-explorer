<script language="javascript">
	...
	function validasiIn(form){
	  if (form.namaMateri.value == ""){
		alert("Anda Belum Memasukkan Nama Materi");
	  ...
	  if (form.fupload.value == ""){
		alert("Anda Belum Memasukkan Berkas Materi");
		form.fupload.focus();
		return (false);
	  }
	  return (true);
...
$aksi="modul/modGMT/aksiGmt.php";
switch($_GET['act']){
	//Hapus User
	case "delete":
	$tampil = mysql_query("SELECT * FROM materi WHERE id_materi=$_GET[id]");
	$r=mysql_fetch_array($tampil);
	unlink("../files/$r[namafile_materi]");
	...
	$tampil = mysql_query("SELECT * FROM materi,mata_pelajaran,kelas WHERE username='$_SESSION[namauser]' AND mata_pelajaran.id_mapel=materi.id_mapel AND kelas.id_kelas=materi.id_kelas ORDER BY tanggal DESC");
	echo "<input class='button orange' type=button value='Tambah Materi' onclick=\"window.location.href='?module=gmt&act=tambah';\">";
    ...
    while ($r=mysql_fetch_array($tampil)){
       ...
       echo "<tr>
				...
				<td>$r[nama_kelas]</td>
				<td>$tgl</td>
				<td><a href=?module=gmt&act=detail&id=$r[id_materi]><img src='images/detail.png' border='0' title='Detail' height='20' width='20' /></a></td>
				<td><a href=?module=gmt&act=edit&id=$r[id_materi]><img src='images/edit.png' border='0' title='Ubah' height='20' width='20' /></a></td>
				<td><a href=?module=gmt&act=delete&id=$r[id_materi]><img src='images/delete.png' border='0' title='Hapus' height='20' width='20' /></a></td>
			</tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambah":
	...
	echo "<form method=POST action='$aksi?module=gmt&act=input' enctype='multipart/form-data' onSubmit=\"return validasiIn(this)\">
		  <table>
          <tr><th>Nama Materi</th>     <td> : <input class='form-input-gray' type=text id=namaMateri name='namaMateri'></td></tr>
          ...
          <tr><th>Berkas Materi</th> <td> : <input class='formFile' type=file id=fupload name='fupload'></td></tr>  
          <tr><td colspan=2>
  ...
  case "edit":
    $edit=mysql_query("SELECT * FROM materi WHERE id_materi='$_GET[id]'");
	...
	echo "<form method=POST action='$aksi?module=gmt&act=update' enctype='multipart/form-data' onSubmit=\"return validasiUp(this)\">
          <input type=hidden name=id value='$r[id_materi]'>
          <input type=hidden name=fileLama value='$r[namafile_materi]'>
		  <table>
          <tr><th>Nama Materi</th>     <td> : <input class='form-input-gray' type=text id=namaMateri name='namaMateri' value='$r[nama_materi]'>&nbsp;&nbsp;&nbsp;</td></tr>
          ...
          <tr><th>Deskripsi</th>			   <td><textarea class='formText' id=deskripsi name=deskripsi width=100 height=50>$r[deskripsi_materi]</textarea>&nbsp;&nbsp;&nbsp;</td></tr>
          <tr><th>Berkas Materi</th> <td> : <input class='formFile' type=file id=fupload name='fupload'> *)</td></tr>  

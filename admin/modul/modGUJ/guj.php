<script language="javascript">
	...
	function validasiIn(form){
	  if (form.namaUjian.value == ""){
		alert("Anda Belum Memasukkan Nama Ujian");
	  ...
	  if (form.soal.value == ""){
		alert("Anda Belum Memasukkan Uraian Soal Ujian");
		form.soal.focus();
		return (false);
	  }
	  return (true);
...
switch($_GET['act']){
	...
	$tampil = mysql_query("SELECT * FROM ujian,mata_pelajaran,kelas WHERE username='$_SESSION[namauser]' AND mata_pelajaran.id_mapel=ujian.id_mapel AND kelas.id_kelas=ujian.id_kelas ORDER BY tanggal DESC");
	echo "<input class='button orange' type=button value='Tambah Ujian' onclick=\"window.location.href='?module=guj&act=tambah';\">";
    ...
    while ($r=mysql_fetch_array($tampil)){
       ...
       echo "<tr>
				...
				<td>$r[nama_kelas]</td>
				<td>$tgl</td>
				<td><a href=?module=guj&act=detail&id=$r[id_ujian]><img src='images/detail.png' border='0' title='Detail' height='20' width='20' /></a></td>
				<td><a href=?module=guj&act=edit&id=$r[id_ujian]><img src='images/edit.png' border='0' title='Ubah' height='20' width='20' /></a></td>
				<td><a href=?module=guj&act=delete&id=$r[id_ujian]><img src='images/delete.png' border='0' title='Hapus' height='20' width='20' /></a></td>
			</tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "tambah":
	...
	echo "<form method=POST action='$aksi?module=guj&act=input' enctype='multipart/form-data' onSubmit=\"return validasiIn(this)\">
		  <table>
          <tr><th>Nama Ujian</th>     <td> : <input class='form-input-gray' type=text id=namaUjian name='namaUjian'></td></tr>
		  ...
		  <tr><th>Uraian Soal Ujian</th>	   <td><textarea class='formText' id=soal name=soal width=100 height=50></textarea></td></tr>
          <tr><th>Berkas Soal Ujian</th> <td> : <input class='formFile' type=file id=fupload name='fupload'></td></tr>  
          <tr><td colspan=2>
  ...
  case "edit":
    $edit=mysql_query("SELECT * FROM ujian WHERE id_ujian='$_GET[id]'");
	...
	echo "<form method=POST action='$aksi?module=guj&act=update' enctype='multipart/form-data' onSubmit=\"return validasiUp(this)\">
          <input type=hidden name=id value='$r[id_ujian]'>
          <input type=hidden name=fileLama value='$r[namafile_ujian]'>
		  <table>
          <tr><th>Nama Ujian</th>     <td> : <input class='form-input-gray' type=text id=namaUjian name='namaUjian' value='$r[nama_ujian]'>&nbsp;&nbsp;&nbsp;</td></tr>
          ...
          <tr><th>Deskripsi</th>			   <td><textarea class='formText' id=deskripsi name=deskripsi width=100 height=50>$r[deskripsi_ujian]</textarea>&nbsp;&nbsp;&nbsp;</td></tr>
          <tr><th>Uraian Soal Ujian</th>	   <td><textarea class='formText' id=soal name=soal width=100 height=50>$r[soal_ujian]</textarea>&nbsp;&nbsp;&nbsp;</td></tr>
          <tr><th>Berkas Soal Ujian</th> <td> : <input class='formFile' type=file id=fupload name='fupload'> *)</td></tr>  
          <tr><td colspan=2>*) Apabila Berkas Ujian Tidak Diubah, Dikosongkan Saja.</td></tr>
		  <tr><td colspan=2>
			<input type=submit class='button orange' value=Simpan>
			<input type=button class='button orange' value=Batal onclick=self.history.back()>
		  </td></tr>
	...
	case "detail":
		require_once "modul/modGUJ/detailGuj.php";
    break;
	
	case "detailKumpul":
		require_once "modul/modGUJ/detailGujKumpul.php";
    break;
}
}
?>

...
if($halaman_aktif > 1){
	...
	$link_halaman .= "<span class=prevnext><a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=1>First</a></span>
						<span class=prevnext><a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=$prev>Prev</a></span>";
}
else{ 
...
for ($i=$halaman_aktif-2; $i<$halaman_aktif; $i++){
  if ($i < 1)
  	continue;
	  $angka .= "<span class=prevnext><a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=$i>$i</a></span>";
  }
    ...
    if($i > $jmlhalaman)
	  ...
	  $angka .= ($halaman_aktif+2<$jmlhalaman ? " ... | <a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=$jmlhalaman>$jmlhalaman</a> | " : " ");
...
if($halaman_aktif < $jmlhalaman){
	...
	$link_halaman .= "<span class=prevnext><a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=$next>Next</a></span>
					<span class=prevnext><a href=$_SERVER[PHP_SELF]?key=$_GET[key]&halaman=$jmlhalaman>Last</a></span>";
}
else{
